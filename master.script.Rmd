...
#HBCA
load("pdx-hbca-integration/hbca_patient_names.txt")
hbca.patient.list<-list()
for (i in 1:length(hbca.patients)){
  hbca.patient.list[[i]]<-assign(paste(hbca.patients[i]),
         readRDS(paste("pdx-hbca-integration/seurat-objects/",hbca.patients[i],"_seurat3_object.rds",sep = "")))
...
DefaultAssay(hbca.combined)<-"integrated"
hci001.anchors <- FindTransferAnchors(reference = hbca.combined, query = hci001.query, 
    dims = 1:30)
predictions <- TransferData(anchorset = hci001.anchors, refdata = hbca.combined$coarse.ident, 
    dims = 1:30)
hci001.query <- AddMetaData(hci001.query, metadata = predictions)

hci002.query <- p02.counts
DefaultAssay(hbca.combined)<-"integrated"
hci002.anchors <- FindTransferAnchors(reference = hbca.combined, query = hci002.query, 
    dims = 1:30)
predictions <- TransferData(anchorset = hci002.anchors, refdata = hbca.combined$coarse.ident, 
    dims = 1:30)
hci002.query <- AddMetaData(hci002.query, metadata = predictions)

hci010.query <- p10.counts
DefaultAssay(hbca.combined)<-"integrated"
hci010.anchors <- FindTransferAnchors(reference = hbca.combined, query = hci010.query, 
    dims = 1:30)
predictions <- TransferData(anchorset = hci010.anchors, refdata = hbca.combined$coarse.ident, 
    dims = 1:30)
