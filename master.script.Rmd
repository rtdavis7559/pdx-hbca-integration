...
```{r label.transfer.identify.transfer.genes, eval=TRUE, echo=TRUE}
...
hci001.anchors.plot<-FetchData(hci001.query, c("tissue",
                                                  ...
                                                  "predicted.id",
                                                  "cell.names",
                                                  hci001.gene.anchors), slot = "data")
levels(hci001.anchors.plot$tissue)<-c("Metastatic","Tumor")

hci001.anchors.heatmap.labels<-hci001.anchors.plot[,1:2]
colnames(hci001.anchors.heatmap.labels)<-c("Status","Paper ID")

colors<-brewer.pal(5, "Dark2")
hci001.anchors.heatmap.colors<-list("Status" = c("Tumor"=colors[1],"Metastatic"=colors[2]),
                                       "Paper ID" = c("A1"=colors[3],"A2"=colors[4],"A3"=colors[5]))
...
pheatmap((as.matrix(hci001.anchors.heatmap)),
         ...
         cluster_rows = mat_cluster_rows,
         color = PurpleAndYellow(50),
         annotation_col = hci001.anchors.heatmap.labels,
         annotation_colors = hci001.anchors.heatmap.colors,
         cutree_cols = 1,
         show_rownames = FALSE)
```
